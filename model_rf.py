...
# 5. 贝叶斯调参（只在训练集上）- 固定bootstrap=False并增加正则化
search_space = {
    'n_estimators': Integer(300, 700, prior='log-uniform'),
    'max_depth': Integer(18, 25),
...
model = RandomForestRegressor(random_state=seed, n_jobs=14)
...
def save_results_with_metrics_and_params(y_train_true, y_train_pred, y_test_true, y_test_pred,
    ...
    if imputer_info:
        ...
        params_data.append(['n_neighbors', imputer_info.get('n_neighbors', 5)])
...
# 保存所有结果
output_path = r"E:\dm\result\rf.xlsx"
imputer_info = {
    'n_neighbors': 10,
    'weights': 'uniform'
}
